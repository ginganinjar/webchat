$(document).ready(function () {
    ...
    if ($newUser != "") {
      ...
      socket.emit("new user", $newUser, function (data) {
        if (data) {
          ...
          $("#loginMsg").html(
            "Sorry! A user by that name already exists. Try some other name."
          );
        }
      });
    } else {
  ...
  $chatForm.on("submit", function (e) {
    e.preventDefault();
    var msg = $("#message").val();
    if (msg != "") socket.emit("new message", msg);
    $("#message").val("");
  });
  ...
  $("#connect").on("click", function () {
    console.log(
      "Trying to start a call. Current call started status :" + isStarted
    );
    if (!isStarted) {
      ...
      if (remoteUser != "" && remoteUser != self) {
        ...
        socket.emit(
          "newVideoChatRequest",
          { sender: self, receiver: remoteUser },
          function (data) {
            $("#remoteUser").val("");
            if (data.response) {
              console.log("Your call was accepted!");
              $("#callStatus").html(
                "Call accepted. Initiating video call now. Please, Allow Media Access to continue."
              );
              $("#video-chat")
                .children("h3")
              ...
              if (navigator.getUserMedia) {
                ...
                $("#callStatus").html(
                  "Your browser does not support getUserMedia. Please update your broswer to use this app."
                );
              isStarted = true;
            } else {
              console.log(
                "Your call request was either rejected or the user is busy"
              );
              $("#callStatus").html("Call Failed. Reason: " + data.reason);
            }
          }
        );
      } else alert("Please enter a valid remote user name");
    } else {
  ...
  socket.on("newVideoCallRequest", function (data, callback) {
    console.log(
      "New Video Call Request. Current call started status :" + isStarted
    );
    if (!isStarted) {
      $div = $(".callRequest");
      ...
      $div.on("click", ".green", function () {
        ...
        $("#callStatus").html(
          "Call accepted. Initiating video call now. Please, allow Media Access when asked for."
        );
        navigator.getUserMedia(
          { video: true, audio: true },
          answererSuccess,
        ...
        function answererSuccess(mediaStream) {
          ...
          pc.onaddstream = function (streamEvent) {
            ...
            remoteVideoElement.src = webkitURL.createObjectURL(
              streamEvent.stream
            );
            $("#callStatus").html("Call in progress...");
          };
